...
cusps := Cusps(X);
printf "CUSPS number=%o degrees=%o\n", #cusps, Sort([Degree(c) : c in cusps]);
...
assert &+[d[1] : d in cuspdata] eq &+[Degree(c) : c in cusps];   // same number of geometric cusps
...
printf "QLIST all (used when there is no cuspidal part): %o\n", Qs;
...
reps := [ PlacesUpToDiamond(X, places[d]) : d in [1..D] ];
...
for dd in [1..D] do
  ...
  for lambda in Partitions(dd) do
    ...
    for Dv in divs do
      ...
      for q in qlist do
        try
          AqD := HeckeOperator(X, q, Dv) - q*DiamondOperator(X, q, Dv) - Dv;
          if not IsPrincipal(AqD) then killed := true; break; end if;
        catch e
